...
abc_T_TS_ammonia <- read.csv(file=file.path(Standards_Table_Folder_location,
                                            "Ammonia",
                                            "ClassABC_TotalAmmonia_AquatChron_W_T_TS_specification.csv"),
                             header = TRUE,
                             sep = ",",
                             stringsAsFactors = FALSE,
                             as.is = TRUE)
...
## Table of aquatic chronic ammonia standards for class A,B,C waterbodies w/Trout or Trout Spawning classification
# abc_wOut_T_TS_ammonia_uia <- read.csv(file=file.path(Standards_Table_Folder_location,
...
abc_wOut_T_TS_ammonia <- read.csv(file=file.path(Standards_Table_Folder_location,
                                                 "Ammonia",
                                                 "ClassABC_TotalAmmonia_AquatChron_Wout_T_TS_specification.csv"),
                                  header = TRUE,
                                  sep = ",",
                                  stringsAsFactors = FALSE,
                                  as.is = TRUE)
...
D_wOut_TS_ammonia <- read.csv(file=file.path(Standards_Table_Folder_location,
                                             "Ammonia",
                                             "ClassD_TotalAmmonia_AquatChron_Wout_T_TS_specification.csv"),
                              header = TRUE,
                              sep = ",",
                              stringsAsFactors = FALSE,
                              as.is = TRUE)
...
if (analysis_lab=="ALS"){  
  ...
  data <- merge(data, data2, by=c("Project_name", "SiteID", "BASIN", "LOCATION", "RIVMILE", "sample", "compare_col", "SAMPLE_DATE", "cas_rn", "fraction", "Parameter.Names", "p_sample", "CLASSIFICA", "STANDARD", "PWL_ID"))
  data$VALUE.x <- data$VALUE.y
  data$UNITS.x <- data$UNITS.y
  
  # Keep only desired columns
  keep <- c("Project_name", "SiteID", "BASIN", "LOCATION", "RIVMILE", "SAMPLE_DATE", "cas_rn", "fraction", "Parameter.Names", "CLASSIFICA", "STANDARD", "PWL_ID", "VALUE.x", "UNITS.x", "sample", "compare_col", "p_sample")
  data <- data[keep]
  rm(keep, data2)
...
######## START ########  EDITS FOR ADDING IN FRACTION FIELD INTO COMPARISONS ########   
      ...
      sampleoutput <- merge(sampleoutput,sample,by=c("Project_name", "SiteID", "BASIN","LOCATION","RIVMILE","SAMPLE_DATE","CLASSIFICA","STANDARD","PWL_ID","UNITS", "VALUE","sample","p_sample", "compare_col"),all=TRUE)
      ...
      keep <- c("Project_name", "SiteID", "BASIN","LOCATION","RIVMILE","SAMPLE_DATE","Parameter.Names","compare_col", "cas_rn", "fraction", "CLASSIFICA","STANDARD","PWL_ID","UNITS","VALUE","sample","p_sample",
...
######## END ########  EDITS FOR ADDING IN FRACTION FIELD INTO COMPARISONS ########  
    ...
    if(water_ammonia_as_N %in% sample[[parameter_column]]){
      ...
      # Verify that a water temperature value is present corresponding with the ammonia value
      if(water_temperature %in% sample[[parameter_column]]){
        water_temp <- sample$VALUE[sample[[parameter_column]]==water_temperature]
        
        # Verify that there is a pH value present corresponding with the ammonia value
        if(water_ph %in% sample[[parameter_column]]){
        
          # Isolate the next lower and higher pH value for getting standard table values
          ph_1 <- ifelse(pH_value>=6.5 & pH_value<6.75, 6.5, 
                         ifelse(pH_value>=6.75 & pH_value<7, 6.75, 
                                ifelse(pH_value>=7 & pH_value<7.25, 7, 
                                       ifelse(pH_value>=7.25 & pH_value<7.5, 7.25, 
                                              ifelse(pH_value>=7.5 & pH_value<7.75, 7.5, 
                                                     ifelse(pH_value>=7.75 & pH_value<8, 7.75, 
                                                            ifelse(pH_value>=8 & pH_value<=8.25, 8, 
                                                                   ifelse(pH_value>=8.25 & pH_value<8.5, 8.25, 
                                                                          ifelse(pH_value>=8.5 & pH_value<8.75, 8.5, 
                                                                                 ifelse(pH_value>=8.75 & pH_value<9, 8.75,
                                                                                        ifelse(pH_value==9, 9,
                                                                                               "pH out of bounds")))))))))))
          ph_2 <- ifelse(pH_value==6.5, 6.5, 
                         ifelse(pH_value>6.5 & pH_value<=6.75, 6.75, 
                                ifelse(pH_value>6.75 & pH_value<=7, 7, 
                                       ifelse(pH_value>7 & pH_value<=7.25, 7.25, 
                                              ifelse(pH_value>7.25 & pH_value<=7.5, 7.5, 
                                                     ifelse(pH_value>7.5 & pH_value<=7.75, 7.75, 
                                                            ifelse(pH_value>7.75 & pH_value<=8, 8, 
                                                                   ifelse(pH_value>8 & pH_value<=8.25, 8.25, 
                                                                          ifelse(pH_value>8.25 & pH_value<=8.5, 8.5, 
                                                                                 ifelse(pH_value>8.5 & pH_value<=8.75, 8.75, 
                                                                                        ifelse(pH_value>8.75 & pH_value<=9, 9, 
                                                                                               "pH out of bounds")))))))))))
          
          # Isolate the next lower and higher water temp value for getting standard table values
          temp_1 <- ifelse(water_temp>=0 & water_temp<5, 0, 
                           ifelse(water_temp>=5 & water_temp<10, 5, 
                                  ifelse(water_temp>=10 & water_temp<15, 10, 
                                         ifelse(water_temp>=15 & water_temp<20, 15, 
                                                ifelse(water_temp>=20 & water_temp<25, 20 , 
                                                       ifelse(water_temp>=25 & water_temp<30, 25, 
                                                              ifelse(water_temp==30, 30,
                                                                     "water temp out of bounds")))))))
          
          temp_2 <- ifelse(water_temp==0, 0, 
                           ifelse(water_temp>0 & water_temp<=5, 5, 
                                  ifelse(water_temp>5 & water_temp<=10, 10, 
                                         ifelse(water_temp>10 & water_temp<=15, 15, 
                                                ifelse(water_temp>15 & water_temp<=20, 20, 
                                                       ifelse(water_temp>20 & water_temp<=25, 25 , 
                                                              ifelse(water_temp>25 & water_temp<=30, 30, 
                                                                     "water temp out of bounds")))))))
          
          # Compare Total Ammonia for Class A, B, & C Non-Trout and Non-Trout spawning streams to aquatic standards
          if(sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="A" | sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="AA" |
            ...
            # Identify the standards values to use in interpolation of standard
  
            # Isolate standard table columns and standard values using water temperature column and pH row assignment values
            # pH low value and temp low value - table standard
            ammonia_standard_x1 <- ifelse(temp_1==0, abc_wOut_T_TS_ammonia$X0.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                          ifelse(temp_1==5, abc_wOut_T_TS_ammonia$X5.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                 ifelse(temp_1==10, abc_wOut_T_TS_ammonia$X10.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                        ifelse(temp_1==15, abc_wOut_T_TS_ammonia$X15.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                               ifelse(temp_1==20, abc_wOut_T_TS_ammonia$X20.C[abc_wOut_T_TS_ammonia$pH==ph_1] , 
                                                                      ifelse(temp_1==25, abc_wOut_T_TS_ammonia$X25.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                                             ifelse(temp_1==30, abc_wOut_T_TS_ammonia$X30.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                                                    "water temp out of bounds")))))))
  
            # pH low value and temp high value - table standard
            ammonia_standard_x2 <- ifelse(temp_2==0, abc_wOut_T_TS_ammonia$X0.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                          ifelse(temp_2==5, abc_wOut_T_TS_ammonia$X5.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                 ifelse(temp_2==10, abc_wOut_T_TS_ammonia$X10.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                        ifelse(temp_2==15, abc_wOut_T_TS_ammonia$X15.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                               ifelse(temp_2==20, abc_wOut_T_TS_ammonia$X20.C[abc_wOut_T_TS_ammonia$pH==ph_1] , 
                                                                      ifelse(temp_2==25, abc_wOut_T_TS_ammonia$X25.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                                             ifelse(temp_2==30, abc_wOut_T_TS_ammonia$X30.C[abc_wOut_T_TS_ammonia$pH==ph_1], 
                                                                                    "water temp out of bounds")))))))
            
            # pH high value and temp low value - table standard
            ammonia_standard_x3 <- ifelse(temp_1==0, abc_wOut_T_TS_ammonia$X0.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                          ifelse(temp_1==5, abc_wOut_T_TS_ammonia$X5.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                 ifelse(temp_1==10, abc_wOut_T_TS_ammonia$X10.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                        ifelse(temp_1==15, abc_wOut_T_TS_ammonia$X15.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                               ifelse(temp_1==20, abc_wOut_T_TS_ammonia$X20.C[abc_wOut_T_TS_ammonia$pH==ph_2] , 
                                                                      ifelse(temp_1==25, abc_wOut_T_TS_ammonia$X25.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                                             ifelse(temp_1==30, abc_wOut_T_TS_ammonia$X30.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                                                    "water temp out of bounds")))))))
            
            # pH high value and temp high value - table standard
            ammonia_standard_x4 <- ifelse(temp_2==0, abc_wOut_T_TS_ammonia$X0.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                          ifelse(temp_2==5, abc_wOut_T_TS_ammonia$X5.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                 ifelse(temp_2==10, abc_wOut_T_TS_ammonia$X10.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                        ifelse(temp_2==15, abc_wOut_T_TS_ammonia$X15.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                               ifelse(temp_2==20, abc_wOut_T_TS_ammonia$X20.C[abc_wOut_T_TS_ammonia$pH==ph_2] , 
                                                                      ifelse(temp_2==25, abc_wOut_T_TS_ammonia$X25.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                                             ifelse(temp_2==30, abc_wOut_T_TS_ammonia$X30.C[abc_wOut_T_TS_ammonia$pH==ph_2], 
                                                                                    "water temp out of bounds")))))))
  
            # Interpolate the standard value using the four isolated table values
            if (is.numeric(ammonia_standard_x1) & is.numeric(ammonia_standard_x2) & is.numeric(ammonia_standard_x3) & is.numeric(ammonia_standard_x4)){
            interpol_standard <- ((ammonia_standard_x1+ammonia_standard_x2+ammonia_standard_x3+ammonia_standard_x4)/4)
            ...
            # Compare total ammonia to interpolated standard value
            sampleoutput$AC[sampleoutput[[parameter_column]]==water_ammonia_as_N &
                              sampleoutput$p_sample[sampleoutput[[parameter_column]]==water_ammonia_as_N]==sample$p_sample[sample[[parameter_column]]==water_ammonia_as_N]] <- ifelse(Ammonia_total>=interpol_standard, "V",
                                                                                                                                                                                              ifelse(Ammonia_total<interpol_standard, "NV",
                                                                                                                                                                                                     ifelse(is.na(Ammonia_total), "Ammonia total is NA",
                                                                                                                                                                                                            ifelse(is.na(interpol_standard), "interpolation error, standard is NA", "error with Ammonia comparison, ammonia and standard are neither >, <, or ="))))
            }
            
            # Handle Non-numeric interpolated standard values
            if (!is.numeric(ammonia_standard_x1) | !is.numeric(ammonia_standard_x2) | !is.numeric(ammonia_standard_x3) | !is.numeric(ammonia_standard_x4)){
              sampleoutput$AC[sampleoutput[[parameter_column]]==water_ammonia_as_N &
          ...
          # Compare total ammonia for Class A, B, & C Trout and Trout spawning streams to aquatic standards
             ...
             sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="C(TS)" | sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="CTS")
          {
            ...
            # pH low value and temp low value - table standard
            ammonia_standard_x1 <- ifelse(temp_1==0, abc_T_TS_ammonia$X0.C[abc_T_TS_ammonia$pH==ph_1], 
                                          ifelse(temp_1==5, abc_T_TS_ammonia$X5.C[abc_T_TS_ammonia$pH==ph_1],
                                                 ifelse(temp_1==10, abc_T_TS_ammonia$X10.C[abc_T_TS_ammonia$pH==ph_1], 
                                                        ifelse(temp_1==15, abc_T_TS_ammonia$X15.C[abc_T_TS_ammonia$pH==ph_1], 
                                                               ifelse(temp_1==20, abc_T_TS_ammonia$X20.C[abc_T_TS_ammonia$pH==ph_1], 
                                                                      ifelse(temp_1==25, abc_T_TS_ammonia$X25.C[abc_T_TS_ammonia$pH==ph_1],
                                                                             ifelse(temp_1==30, abc_T_TS_ammonia$X30.C[abc_T_TS_ammonia$pH==ph_1],
                                                                                    "water temp out of bounds")))))))
            # pH low value and temp high value - table standard
            ammonia_standard_x2 <- ifelse(temp_2==0, abc_T_TS_ammonia$X0.C[abc_T_TS_ammonia$pH==ph_1], 
                                          ifelse(temp_2==5, abc_T_TS_ammonia$X5.C[abc_T_TS_ammonia$pH==ph_1], 
                                                 ifelse(temp_2==10, abc_T_TS_ammonia$X10.C[abc_T_TS_ammonia$pH==ph_1], 
                                                        ifelse(temp_2==15, abc_T_TS_ammonia$X15.C[abc_T_TS_ammonia$pH==ph_1], 
                                                               ifelse(temp_2==20, abc_T_TS_ammonia$X20.C[abc_T_TS_ammonia$pH==ph_1], 
                                                                      ifelse(temp_2==25, abc_T_TS_ammonia$X25.C[abc_T_TS_ammonia$pH==ph_1], 
                                                                             ifelse(temp_2==30, abc_T_TS_ammonia$X30.C[abc_T_TS_ammonia$pH==ph_1], 
                                                                                    "water temp out of bounds")))))))
            # pH high value and temp low value - table standard
            ammonia_standard_x3 <- ifelse(temp_1==0, abc_T_TS_ammonia$X0.C[abc_T_TS_ammonia$pH==ph_2], 
                                          ifelse(temp_1==5, abc_T_TS_ammonia$X5.C[abc_T_TS_ammonia$pH==ph_2], 
                                                 ifelse(temp_1==10, abc_T_TS_ammonia$X10.C[abc_T_TS_ammonia$pH==ph_2],
                                                        ifelse(temp_1==15, abc_T_TS_ammonia$X15.C[abc_T_TS_ammonia$pH==ph_2], 
                                                               ifelse(temp_1==20, abc_T_TS_ammonia$X20.C[abc_T_TS_ammonia$pH==ph_2], 
                                                                      ifelse(temp_1==25, abc_T_TS_ammonia$X25.C[abc_T_TS_ammonia$pH==ph_2],
                                                                             ifelse(temp_1==30, abc_T_TS_ammonia$X30.C[abc_T_TS_ammonia$pH==ph_2], 
                                                                                    "water temp out of bounds")))))))
            
            # pH high value and temp high value - table standard
            ammonia_standard_x4 <- ifelse(temp_2==0, abc_T_TS_ammonia$X0.C[abc_T_TS_ammonia$pH==ph_2], 
                                          ifelse(temp_2==5, abc_T_TS_ammonia$X5.C[abc_T_TS_ammonia$pH==ph_2], 
                                                 ifelse(temp_2==10, abc_T_TS_ammonia$X10.C[abc_T_TS_ammonia$pH==ph_2], 
                                                        ifelse(temp_2==15, abc_T_TS_ammonia$X15.C[abc_T_TS_ammonia$pH==ph_2], 
                                                               ifelse(temp_2==20, abc_T_TS_ammonia$X20.C[abc_T_TS_ammonia$pH==ph_2], 
                                                                      ifelse(temp_2==25, abc_T_TS_ammonia$X25.C[abc_T_TS_ammonia$pH==ph_2],
                                                                             ifelse(temp_2==30, abc_T_TS_ammonia$X30.C[abc_T_TS_ammonia$pH==ph_2], 
                                                                                    "water temp out of bounds")))))))
          ...
          # Compare total ammonia for Class D Non-Trout and Non-Trout spawning streams to aquatic standards
          if(sample$STANDARD[sample[[parameter_column]]==water_ammonia_as_N]=="D")
            ...
            ammonia_standard_x1 <- ifelse(temp_1==0, D_wOut_TS_ammonia$X0.C[D_wOut_TS_ammonia$pH==ph_1], 
                                          ifelse(temp_1==5, D_wOut_TS_ammonia$X5.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                 ifelse(temp_1==10, D_wOut_TS_ammonia$X10.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                        ifelse(temp_1==15, D_wOut_TS_ammonia$X15.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                               ifelse(temp_1==20, D_wOut_TS_ammonia$X20.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                                      ifelse(temp_1==25, D_wOut_TS_ammonia$X25.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                                             ifelse(temp_1==30, D_wOut_TS_ammonia$X30.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                                                    "water temp out of bounds")))))))
            # pH low value and temp high value - table standard
            ammonia_standard_x2 <- ifelse(temp_2==0, D_wOut_TS_ammonia$X0.C[D_wOut_TS_ammonia$pH==ph_1], 
                                          ifelse(temp_2==5, D_wOut_TS_ammonia$X5.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                 ifelse(temp_2==10, D_wOut_TS_ammonia$X10.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                        ifelse(temp_2==15, D_wOut_TS_ammonia$X15.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                               ifelse(temp_2==20, D_wOut_TS_ammonia$X20.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                                      ifelse(temp_2==25, D_wOut_TS_ammonia$X25.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                                             ifelse(temp_2==30, D_wOut_TS_ammonia$X30.C[D_wOut_TS_ammonia$pH==ph_1], 
                                                                                    "water temp out of bounds")))))))
            
            # pH high value and temp low value - table standard
            ammonia_standard_x3 <- ifelse(temp_1==0, D_wOut_TS_ammonia$X0.C[D_wOut_TS_ammonia$pH==ph_2],
                                          ifelse(temp_1==5, D_wOut_TS_ammonia$X5.C[D_wOut_TS_ammonia$pH==ph_2],
                                                 ifelse(temp_1==10, D_wOut_TS_ammonia$X10.C[D_wOut_TS_ammonia$pH==ph_2],
                                                        ifelse(temp_1==15, D_wOut_TS_ammonia$X15.C[D_wOut_TS_ammonia$pH==ph_2],
                                                               ifelse(temp_1==20, D_wOut_TS_ammonia$X20.C[D_wOut_TS_ammonia$pH==ph_2],
                                                                      ifelse(temp_1==25, D_wOut_TS_ammonia$X25.C[D_wOut_TS_ammonia$pH==ph_2],
                                                                             ifelse(temp_1==30, D_wOut_TS_ammonia$X30.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                                                    "water temp out of bounds")))))))
            
            # pH high value and temp high value - table standard
            ammonia_standard_x4 <- ifelse(temp_2==0, D_wOut_TS_ammonia$X0.C[D_wOut_TS_ammonia$pH==ph_2],
                                          ifelse(temp_2==5, D_wOut_TS_ammonia$X5.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                 ifelse(temp_2==10, D_wOut_TS_ammonia$X10.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                        ifelse(temp_2==15, D_wOut_TS_ammonia$X15.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                               ifelse(temp_2==20, D_wOut_TS_ammonia$X20.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                                      ifelse(temp_2==25, D_wOut_TS_ammonia$X25.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                                             ifelse(temp_2==30, D_wOut_TS_ammonia$X30.C[D_wOut_TS_ammonia$pH==ph_2], 
                                                                                    "water temp out of bounds")))))))
          ...
          }#<- Closes pH function in ammonia comparison
...
if (analysis_lab == "ALS"){
  ...
  if (nrow(chem_data)==nrow(finalOutput)){
    match_col <- intersect( colnames(finalOutput),  colnames(chem_data))
    #finalOutput <- merge(finalOutput, chem_data, by=match_col, all.x = TRUE)
